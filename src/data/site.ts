export const site = {
  name: "Cast Right Catch Co.",
  shortName: "Cast Right Catch",
  tagline: "Following the Right Direction. Delivering the Right Catch.",
  guidedBy: "Guided by honesty, wisdom, and trusted partnerships in every catch.",
  description:
    "Cast Right Catch Co. partners with seafood buyers domestically and globally, providing premium seafood while simplifying the logistics that keep the supply chain moving.",
  url: "https://www.castrightcatch.com",
  email: "info@castrightcatch.com",
  formEmail: "info@castrightcatch.com",
  founded: "June 2026",
};

export const nav = [
  { href: "/", label: "Home" },
  { href: "/about", label: "About" },
  { href: "/contact", label: "Contact" },
] as const;
